...
def linuxConfig():
    ...
    if os.path.isdir(variables.dir_data) == False:
        ...
        os.system(f"mkdir {variables.dir_data}")
    ...
    if os.path.isfile(f"{variables.dir_data}/sourceImage.png") == False:
        ...
        img.save(f"{variables.dir_data}/sourceImage.png")
    ...
    if os.path.isfile(f"{variables.dir_data}/extractedText.txt") == False:
        os.system("echo [91m    Could not find extractedText.txt[0m")

        os.system(f"echo >nul > {variables.dir_data}/extractedText.txt")
        if os.path.isfile(f"{variables.dir_data}/extractedText.txt") == False:
